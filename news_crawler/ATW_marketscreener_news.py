...
def _extract_row_date(anchor: Tag) -> str:
    """Walk up to the nearest <tr>/<div> row, then try every date source."""
    row = anchor.find_parent("tr") or anchor.find_parent("div") or anchor.parent
    if row is None:
        return ""
    date_span = row.find("span", attrs={"data-utc-date": True})
    if date_span:
        parsed = parse_date(date_span["data-utc-date"])
        if parsed:
            return parsed[:10]
    any_utc = row.find(attrs={"data-utc-date": True})
    if any_utc:
        parsed = parse_date(any_utc.get("data-utc-date"))
        if parsed:
            return parsed[:10]
    ...
    french = parse_french_date(row_text)
    if french:
        return french
    return ""
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Standalone run: MarketScreener ATW")
