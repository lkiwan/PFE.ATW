...
Much cleaner than scraping the topic HTML page: structured JSON, reliable
...
def scrape(
    ...
    if known_url_keys is not None and links:
        if url_key(links[0]) in known_url_keys:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Standalone run: Medias24 ATW")
    parser.add_argument("--show", type=int, default=10, help="How many items to print")
    parser.add_argument("--max-pages", type=int, default=3, help="WP REST pages to fetch")
