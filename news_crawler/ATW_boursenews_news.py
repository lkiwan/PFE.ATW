...
The page `/action/attijariwafa-bank` is already scoped to ATW, so every
article link is implicitly on-topic. We accept any `/article/...` href
(not just `/article/marches/`) and try to extract a date from the
surrounding DOM before falling back to body enrichment.
"""
...
def _extract_date_near(anchor: Tag) -> str:
    """Walk up from an article link and try to find a date in the surrounding
    block. Returns an ISO-ish date string, or empty string if nothing found.
    """
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Standalone run: Boursenews ATW")
