"""Google News RSS — broad discovery fallback.

Off by default in the orchestrator because gnewsdecoder is slow (~3s per
URL resolution). Enable via `--with-gnews` to catch international coverage
the direct ATW-specific sources miss.
"""
...
def scrape(known_url_keys: Optional[set[str]] = None) -> list[dict]:
    """Run all configured Google News queries and merge the results."""
    items: list[dict] = []
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Standalone run: Google News ATW")
